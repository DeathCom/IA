...
def vrp_voraz():
	...
	for k,v in s:
		...
		if rc1==None and rc2==None:
			#no esta en ninguna ruta, creamos la ruta
			if peso_ruta([k[0], k[1]]) <= max_carga:
				rutas.append([k[0],k[1]])

		elif rc1!=None and rc2==None:
			#ciudad 1 ya esta en la ruta, agregamos ciudad 2
			if rc1[0]==k[0]:
			...
			#Ciudad 2 ya esta en la ruta, agregamos ciudad 1
			if rc2[0]==k[1]:
			...
			#Ciudad 1 y2 ya estan en la ruta, tratamos de unirlas
...
if __name__ == '__main__':
	...
	pedidos={
		'Malaga':10, 'Sevilla':13, 'Granada':7, 'Valencia':11, 'Madrid':15,
		'Salamanca':8, 'Santiago':6, 'Santander':7, 'Zaragoza':8, 'Barcelona':14
	}
